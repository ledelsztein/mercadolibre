...
- **Costo abierto en dos** (pedido de Lucas, 2026-09-25): `costo_pagina_*` = campaña de Seguidores de Mi página (`CDLIT`, y su anulación `BDLIT`) -> línea "Publicidad Página" del P&L; `costo_ventas_*` = todo el resto de `MCLICS` (Product Ads `PADS`, Display, etc.) -> línea "Publicidad Ventas". `costo_*` sigue siendo el total. Las dos son Costo Fijo en el punto de equilibrio.
- **Las anulaciones (`detail_type == "BONUS"`, ej. `BDLIT`) restan.** Hasta 2026-09-25 se sumaban por error: junio tuvo $53.5K de `BDLIT` contados en positivo, y la publicidad de mayo/junio quedaba inflada ~$107K. Al corregirlo se borró `periodos_cerrados` para re-pedir todo.
