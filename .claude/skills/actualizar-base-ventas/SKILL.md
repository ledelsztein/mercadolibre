...
- **Solo cargos que paga Lucas** (`accounts.from == "collector"` en `charges_details`). Error real corregido 2026-09-23 (orden 2000018573593808): el `financing_fee` de cuotas lo paga el **comprador** (`from: payer -> mp`) y se estaba sumando como cargo variable ($36.598 de más en esa orden). Idem `coupon_code` (`from: ml -> payer`), que es un descuento que financia ML.
- **Cargo por cupón** (`type=="coupon"`, `coupon_fee`, `from: collector -> ml`): lo que ML le cobra a Lucas por un cupón que usó el comprador. Va en **columna aparte** "Cargo por cupón" (pedido de Lucas 2026-09-23), resta en Importe recibido, y en el P&L es una línea aparte (`cargo_cupon`) que suma a Total Egresos y a Costo Variable. Sin IVA = ÷1,21. Solo se reprocesó septiembre con esta regla (decisión de Lucas) -- mayo–agosto tienen el cupón en 0 y pueden tener cargo de cuotas del comprador inflado; si hace falta, reprocesar esos meses.
